SELECT
    ...
    0 AS Quantity,
    CASE
        WHEN T.CurrOrders = 0 THEN 0
        ELSE T.CurrRevenue / T.CurrOrders
    END AS AOV,

/* --- 2. TINH TANG TRUONG (GROWTH) --- */

/* Revenue Growth */
CASE
    WHEN T.PrevRevenue = 0
    AND T.CurrRevenue > 0 THEN 100
    WHEN T.PrevRevenue = 0 THEN 0
    ELSE (
        (T.CurrRevenue - T.PrevRevenue) / T.PrevRevenue
    ) * 100
...
CASE
    WHEN T.PrevOrders = 0
    AND T.CurrOrders > 0 THEN 100
    WHEN T.PrevOrders = 0 THEN 0
    ELSE (
        (T.CurrOrders - T.PrevOrders) / T.PrevOrders
    ) * 100
END AS OrdersGrowth,
...
COUNT(
    DISTINCT CASE
        WHEN CreatedTime BETWEEN % s AND %  s THEN OmisellOrderNumber
    END
) AS CurrOrders,
...
SUM(
    CASE
        WHEN CreatedTime BETWEEN % s AND %  s THEN (
            OriginalPrice - DiscountSeller - VoucherSeller
        ) * Quantity
        ELSE 0
    END
) AS PrevRevenue,
